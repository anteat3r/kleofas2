...
class _EventsPageState extends State<EventsPage> {
  final List<String> czWeekDayNames = [
    'Ne',
    'Po',
    'Út',
    'St',
  ...
  Widget dayCell(Map day, double maxHeight) {
    ...
    events = events
        .where((element) => element['Times']
            .map((element1) => element1['StartTime'].split('T')[0])
            .contains(day['Date'].split('T')[0]))
        .toList();
    return SizedBox(
      ...
      child: ElevatedButton(
          onPressed: () {
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        DayPage(DateTime.parse(day['Date']))));
          },
          onLongPress: () {
            showDialog(
                context: context,
            ...
            children: [
              ...
              Text(
                DateFormat('d. M.').format(
                    DateTime.parse(day['Date'])..add(const Duration(days: 1))),
                style: const TextStyle(
                  fontWeight: FontWeight.w500,
                  fontSize: 12,
                ),
  ...
  Widget hourCell(Map? hour, double maxHeight, Map subjects, Map rooms,
      Map teachers, Map groups) {
    return SizedBox(
      height: maxHeight * 6 / 35,
      child: ElevatedButton(
          onPressed: (hour == null)
          ...
          style: ButtonStyle(
            shape: MaterialStatePropertyAll(RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(10))),
            backgroundColor: MaterialStatePropertyAll(hour?['Change'] != null
                ? Colors.lightBlue
    ...
    AppBar appBar = AppBar(
      ...
      actions: [
        IconButton(
            onPressed: () {
              setState(() async {
                String cookie = await loginWebCookie(
                    getPassword("bakalari", "url"),
                    getPassword("bakalari", "username"),
                    getPassword("bakalari", "password"));
                String rawHtml = await queryWeb(getPassword("bakalari", "url"),
                    'Timetable/Public/Actual/Class/ZX', cookie);
                var htmlDocument = parseHtmlDocument(rawHtml);
                List output = [];
                for (html.Element row
                    in htmlDocument.querySelectorAll('.bk-timetable-row')) {
                  String dateStr =
                      row.querySelector(".bk-day-day")?.text ?? 'idk';
                  String dateNum =
                      row.querySelector(".bk-day-date")?.text ?? 'idk';
                  output
                      .add({"weekday": dateStr, "date": dateNum, "atoms": []});
                  for (html.Element hour in row
                          .querySelector(".bk-cell-wrapper")
                          ?.querySelectorAll('.bk-timetable-cell') ??
                      []) {
                    for (html.Element dtDetail
                        in hour.querySelectorAll(".day-item-hover")) {
                      Map dtDetailStr = jsonDecode(
                          dtDetail.getAttribute("data-detail") ?? '[]');
                      String subject =
                          dtDetail.querySelector(".middle")?.text ?? "idk";
                      int hourNum = int.parse(
                          RegExp(r"\| (\d+) \(\d+:\d+ - \d+:\d+\)")
                                  .allMatches(dtDetailStr["subjecttext"])
                                  .first
                                  .group(1) ??
                              '-1');
                      String room = dtDetailStr['room'] ??
                          "idk"; //dtDetail.querySelector('.right')?.firstChild?.text ?? 'idk';
                      String group = dtDetailStr['group'] ??
                          "idk"; //dtDetail.querySelector('.left')?.firstChild?.text ?? 'idk';
                      String teacher = dtDetailStr['teacher'] ??
                          "idk"; //dtDetail.querySelector('.bottom')?.firstChild?.text ?? 'idk';
                      String changeinfo = dtDetailStr['changeinfo'] ?? "idk";
                      String theme = dtDetailStr['theme'] ?? "idk";
                      output.last["atoms"].add({
                        "subject": subject,
                        "hourNumber": hourNum,
                        "room": room,
                        "group": group,
        ...
        children: [
          ValueListenableBuilder(
              ...
              builder: (BuildContext context, Box<Map> value, child) {
                List getElem(String name) =>
                    value.get('timetable')?[name] ?? [];
                List hours = getElem('Hours');
                List days = getElem('days');
                hours = hours.sublist(
                    max(
                        0,
                        hours.indexWhere((element) => days.any((day) =>
                            day['Atoms'].any(
                                (atom) => atom['HourId'] == element['Id'])))),
                    hours.lastIndexWhere((element) => days.any((day) =>
                            day['Atoms'].any(
                                (atom) => atom['HourId'] == element['Id']))) +
                        1);
                        ...
                        ...List.generate(days.length, (index) {
                          return TableRow(
                            children: [
                              ...
                              ...List.generate(hours.length, (index2) {
                                return hourCell(
                                    mapListToMap(days[index]['Atoms'],
                                        id: 'hourNumber')[hours[index2]['Id']],
                                    maxHeight,
                                    subjects,
                                    rooms,
                                    teachers,
