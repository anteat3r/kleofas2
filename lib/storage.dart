...
dynamic jsonify (Object object) {
  if (object.toString().startsWith('{')) {
    return object.cast<Map>().map((key, value) => MapEntry(jsonify(key), jsonify(value)));
  }
  if (object.toString().startsWith('[')) {
    return object.cast<List>().map((element) => jsonify(element)).toList();
  }
  if (object.toString().startsWith('(')) {
    return object.cast<Iterable>().map((element) => jsonify(element)).toList();
  }
  if (object is int || object is double || object is bool) {
...
double? correlCoef(List<double> x, List<double> y) {
  double sumX = 0.0;
  double sumY = 0.0;
  double sumXY = 0.0;
  ...
  try {
    return (x.length*sumXY - sumX*sumY) / sqrt((x.length*squareSumX - sumX*sumX)  *  (x.length*squareSumY - sumY*sumY));
  } catch (_) {
    return null;
  }
}
...
extension EnumerateList<T> on List<T> {
  List<(int, T)> enumerate () => List.generate(length, (i) => (i, this[i]));
}
...
class _TaskDialogState extends State<TaskDialog> {
  ...
  void initState() {
    ...
    if (editing) {
      stream ??= user.get("adminstreams")?.split(",").first;
    }
          ...
          children: [
            ...
            if (editing) Padding(
              ...
              child: DropdownButton<String>(
                ...
                items: user.get("adminstreams")!.split(",").map((e) => DropdownMenuItem(
                  value: e,
                  child: Text(getId(e).abbrev),
                )).toList(),
              ),
            ),
        ...
        if (editing && widget.task?['author'] != null) OutlinedButton(
          /*onPressed: () {
            showDialog(context: context, builder: (context) => const AlertDialog(
              title: Text("Smazat task"),
              content: Text("Pro smazání podrž tlačíko smazání"),
            ),);
          },*/
          onPressed: () {
            loadingSnack(() async {
              ...
              await showDialog(context: context, builder: (context) => AlertDialog(
                ...
                content: const Text("You sure you want to delete this task?"),
                actions: [
                  OutlinedButton(onPressed: () {Navigator.of(context).pop();}, child: const Text("Cancel")),
                  OutlinedButton(onPressed: () async {
                    final navigatorState_ = Navigator.of(context);
                    await loginPb();
                    await pb.collection('tasks').delete(widget.task!['id']);
                    await loadTasks();
                    navigatorState_.pop();
